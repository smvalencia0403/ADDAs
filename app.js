...
function monthTotal(year, month) {
  return transactions
    .filter((t) => isInMonth(txDate(t), year, month))
...
function renderHomeTransactions(filter = "") {
  ...
  filtered.forEach((t) => {
    ...
    row.innerHTML = `
      <div class="tx-icon" style="background:${cat ? cat.fill + "80" : "#E5E5EA"};">${cat ? cat.emoji : "❔"}</div>
      <div class="tx-body">
        <div class="tx-cat">${cat ? cat.name : "Sin categoría"}</div>
        <div class="tx-desc">${t.note || (cat ? cat.name : "Gasto")}</div>
      </div>
      <div class="tx-amount">${fmtMoney(t.amount, 2)}</div>
    `;
    row.onclick = () => {
...
function getPeriodData(period) {
  ...
  if (period === "semana") {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const sum = transactions.filter((t) => isSameDay(new Date(t.date), d)).reduce((s, t) => s + Number(t.amount), 0);
...
document.getElementById("save-tx").addEventListener("click", () => {
  ...
  if (!amount || amount <= 0 || !selectedTxCategoryId) return;
  transactions.push({
    id: uid(),
    categoryId: selectedTxCategoryId,
    amount,
